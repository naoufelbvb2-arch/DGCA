"""
DGCA — RIC-02 & RIC-03
Canonical Host-Side System Runtime (RIC-02 §3, §7-§10; RIC-03 §14-§16)
...
class CanonicalSystemRuntime:
    ...
    lifecycle management, non-cognitive operation serialization (RIC-03 §15), and holding
    CanonicalR1RuntimeRoot, CanonicalChatRuntime, and CanonicalLearningRuntime.
    """

    __slots__ = ("_chat_runtime", "_learning_runtime", "_operation_state", "_runtime_root")
    ...
    @property
    def learning_runtime(self) -> CanonicalLearningRuntime:
        """Canonical learning runtime owned by this system runtime (RIC-03 §14)."""
    ...
    def _acquire_operation(self, state: SystemOperationState) -> None:
        """Acquires exclusive system operation lock or fails closed (RIC-03 §15)."""
        if self._operation_state != SystemOperationState.IDLE:
            if state == SystemOperationState.CHATTING and self._operation_state == SystemOperationState.CHATTING:
                raise RuntimeError(
                    f"Cannot begin operation '{state.value}': runtime is currently in state '{self._operation_state.value}' "
    ...
    def _release_operation(self) -> None:
        """Releases system operation lock, returning state to IDLE (RIC-03 §15)."""
    ...
    def fresh(
        ...
        *,
        ...
        """Executes fresh canonical bootstrap sequence (RIC-02 §7; RIC-03 §14).
        ...
        10. create fresh ephemeral learning capability, authorizer, and runtime
        """
    ...
    def from_checkpoint(
        ...
        *,
        ...
        """Restores a canonical R1 checkpoint into a new CanonicalSystemRuntime (RIC-02 §8; RIC-03 §16)."""
    ...
    def _for_test(
        ...
        *,
        ...
        return cls.fresh(
            ...
            learning_session_nonce=learning_session_nonce,
        )
    ...
    def chat(self, text: str) -> str:
        ...
        Serialized under SystemOperationState.CHATTING (RIC-03 §15).
        """
    ...
    def learn(
        ...
        *,
        ...
        Serialized under SystemOperationState.LEARNING (RIC-03 §15).
        """
    ...
    def save_checkpoint(self, filepath: str | pathlib.Path) -> str:
        ...
        Serialized under SystemOperationState.CHECKPOINTING (RIC-03 §15).
        """
